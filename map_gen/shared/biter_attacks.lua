...
local config = global.config.biter_attacks
...
local function calculate_biters()
    local difficulty = config.timed_attacks.attack_difficulty
...
local function rocket_launched(event)
    ...
    if not config.launch_attacks.first_launch_only and count > 1 then
        --send attack of 1k
        setup_scans(data)
    ...
    function(data)
        ...
        for i = 1, #ents do
            ...
            if biter_count >= biters_to_send then
                Debug.print({biter_count = biter_count, biters_to_send = biters_to_send})
                Attack_data.biter_count = biter_count
                Task.set_timeout(10, attack_command_token, {target_ent = data.target_ent, attack_pos = data.scan_center})
                return false
            end
        end
...
timed_attack_token =
    ...
    function()
        ...
        setup_scans(data)
        Task.set_timeout(config.timed_attacks.attack_frequency, timed_attack_token, {})
    end
)
